...
def parse_error_message(server, volume):
    """ parse list of log files find log file details and return dict """
...
def too_many_logs(server, too_many_list, quiet=False):
    """ Connect to db and insert server, volume serial, and log file details"""
    all_volumes = {}
    for volume in tqdm(too_many_list, disable=quiet, desc='> 2 Errors:'):
        if not database.volume_in_log_files(volume):
            all_volumes[volume] = parse_error_message(server, volume)
    return all_volumes
...
def process(server, quiet=False, rerun=False):
    ...
    logs = parse_logs.parse_logs(output, quiet)
    if logs['archive']:
